...
async def pair_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if len(us) < 1:
        await update.message.reply_text("❗ Provide at least one account. ❗")
        return
...
async def get_messages(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not msgs:
        await update.message.reply_text("⚠️ No messages provided. Send like: hello & buy now & ok ⚠️")
        return MESSAGES
    ...
    # Register runtime-only proc map
